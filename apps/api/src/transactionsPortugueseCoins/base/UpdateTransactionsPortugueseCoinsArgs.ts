/*
------------------------------------------------------------------------------ 
This code was generated by Amplication. 
 
Changes to this file will be lost if the code is regenerated. 

There are other ways to to customize your code, see this doc to learn more
https://docs.amplication.com/how-to/custom-code

------------------------------------------------------------------------------
  */
import { ArgsType, Field } from "@nestjs/graphql";
import { ApiProperty } from "@nestjs/swagger";
import { TransactionsPortugueseCoinsWhereUniqueInput } from "./TransactionsPortugueseCoinsWhereUniqueInput";
import { ValidateNested } from "class-validator";
import { Type } from "class-transformer";
import { TransactionsPortugueseCoinsUpdateInput } from "./TransactionsPortugueseCoinsUpdateInput";

@ArgsType()
class UpdateTransactionsPortugueseCoinsArgs {
  @ApiProperty({
    required: true,
    type: () => TransactionsPortugueseCoinsWhereUniqueInput,
  })
  @ValidateNested()
  @Type(() => TransactionsPortugueseCoinsWhereUniqueInput)
  @Field(() => TransactionsPortugueseCoinsWhereUniqueInput, { nullable: false })
  where!: TransactionsPortugueseCoinsWhereUniqueInput;

  @ApiProperty({
    required: true,
    type: () => TransactionsPortugueseCoinsUpdateInput,
  })
  @ValidateNested()
  @Type(() => TransactionsPortugueseCoinsUpdateInput)
  @Field(() => TransactionsPortugueseCoinsUpdateInput, { nullable: false })
  data!: TransactionsPortugueseCoinsUpdateInput;
}

export { UpdateTransactionsPortugueseCoinsArgs as UpdateTransactionsPortugueseCoinsArgs };
